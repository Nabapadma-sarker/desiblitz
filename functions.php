...
add_action( $hook, function(){
		...
		wp_enqueue_script( 'desiblitz_taboola', get_template_directory_uri() . '/js/taboola.js', false , '');
		
		wp_enqueue_script( 'desiblitz_outbrain', ("https://widgets.outbrain.com/outbrain.js"), false );
	...
	function lym_theme_scripts() {
		...
		wp_enqueue_script( 'easySlider', get_template_directory_uri() . '/js/easySlider1.7.js', array('jquery'),'' ,true );

	}
	add_action( 'wp_enqueue_scripts', 'lym_theme_scripts' );
		
	function lym_enqueue_date_picker($hook){
	  if( 'widgets.php' != $hook )
        return;
	...
	add_action( 'admin_enqueue_scripts', 'lym_enqueue_date_picker' );	
...
add_shortcode( 'post_totes', 'post_totes_function' );
	...
	********************************/
	...
	include (TEMPLATEPATH.'/includes/widgets2.php');
...
add_filter("acf/load_field/name=current_impressions", "readonly_field");